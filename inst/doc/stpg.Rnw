...
This vignette was run in the following session:
<<>>=
library(RPostgreSQL)
library(spacetime)
sessionInfo()
...
The example code in this vignette is not meant as a full-fledged
database access mechanism for spatio-temporal data. In particular,
the selection here can do only {\em one} of spatial locations
(entered as features) or time periods.  If database access is
only based on time, a spatially enabled database (such as PostGIS)
would not be needed.
