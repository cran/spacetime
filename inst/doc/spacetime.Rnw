...
package \code{spacetime} , and is available (in updated form)
as a vignette in this package.  }
...
or interval, thick lines indicate time intervals, arrows indicate movement. Green
codes refer to classes refered to in Section~\ref{classes}.
}
\label{fig:move}
\end{figure}
...
objects with gridded space-time layout (Figure~\ref{fig:st}) deriving
from \code{STF} or \code{STS} assume interval time, where \code{STI}
and \code{STT} objects assume instance time.

As only start times of time intervals are given for all objects
except \code{STL}, the assumption is that when objects have
interval time, time intervals are consecutive (Figure~\ref{fig:move}),
and the last interval (for which no end time is present) has a
length identical to the second last interval.  Objects of class
\code{STL} specify end time explicitly, to allow representation of
arbitrary-length time intervals (Figure~\ref{fig:move}).
...
<<>>=
st = STLDF(geometry(cs), begin, end, as.data.frame(cs))
@
