...
\subsection{Coercion to class {\tt SpatialXxDataFrame}}
Spatio-temporal data objects can be coerced to the corresponding
purely spatial objects.  Objects of class {\tt STFDF} will be 
represented in wide form, where only the first (selected) attribute
is retained:
<<>>=
...
xs1
@
whereas objects of class {\tt STSDF} or {\tt STIDF} will be represented
in long form, where time is added as additional attribute:
<<>>=
xs2 = as(x, "Spatial")
class(xs2)
xs2
