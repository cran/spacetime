...
We use \code{xts} for time because it has nice tools for reorganizing
time and a very flexible syntax to select time periods. We do
not use the \code{xts} objects to store attribute information,
as it is restricted to \code{matrix} objects, and hence can only
store a single type, and not combine numeric and factor. Instead,
as in the classes of \code{sp}, we use \code{data.frame} to store
measured values.
...
time is rich, the data in \code{data.frame} \code{wind} come stations
time series in columns. The station locations come in a separate
\code{data.frame}, called \code{wind.loc}.
<<>>=
wind[1:3,]
wind.loc[1:3,]
@
