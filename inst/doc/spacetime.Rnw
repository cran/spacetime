...
Weseler Strasse 253, 48151 M\"{u}nster, Germany\\
...
including vignettes in package \pkg{spacetime}\footnote{Version
0.7-0 -- an updated version of this paper is found as a vignette
in this package, which implements the classes and methods for
spatio-temporal data described here. } on spatio-temporal overlay
and aggregation, and on using proxy data sets to PostgreSQL tables
when data are too large for R.  Section 9 finishes with a discussion.
...
\item if time reflects time intervals, the interval may not be obvious anymore
...
of its boundaries.
\begin{figure}[htb]
\includegraphics[width=\columnwidth]{move}
\caption{time instant (left, right) or interval (middle), and object movement
(right); $s_1$ refers to the first feature/location, $t_1$ to the first time instance
or interval, thick lines indicate time intervals, arrows indicate movement }
\label{fig:move}
\end{figure}
...
time instance reflects the location at a particular moment, and
movement occurs between registered pairs (time, feature), and must
be continuous.
...
\code{[} & Select spatial and/or temporal sections, and/or data variables \\
...
\code{timeIsInterval} & retrieve or set the flag indicating whether time reflects intervals 
(\code{TRUE}) or instances (\code{FALSE}) \\
\hline
\end{tabular}
\end{center}
...
\begin{Sinput}
Warning message: 
In STF(sp, time) : STF(): setting timeIsInterval to default value TRUE
\end{Sinput}
\end{Schunk}
This warning comes from the fact that during creation,
it is not clear whether the time instances reflect true instances,
or time intervals (section \ref{intervals}). 
The warning can be prevented by setting the
\code{timeIsInterval} attribute of the time component 
prior to creating an \code{ST} object:
<<>>=
...
Objects in \pkg{spacetime} register whether time reflects to
instances or intervals. Not specifying this during creation creates
a warning, mentioning the default that was assumed.  As a default,
objects with gridded space-time layout (figure \ref{fig:st}),
deriving from \code{STF} or \code{STS}, assume interval time,
and other objects assume instance time.  As only start times of
time intervals are given, the package makes the assumption that
the last interval (for which no end time is present) has a length
identical to the one-but-last interval.
...
@
(the output of \code{summary(zz)} is left out for brevity).
...
of graphs from them, shown in the book. It should be noted that
the the book examples only deal with \code{STFDF} objects.
