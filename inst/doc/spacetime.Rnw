...
The goals of this package are to explore how spatio-temporal data can
be sensibly represented in classes, and which methods are useful and
feasible for the classes implemented. It reuses existing classes,
methods, and functions present in packages for spatial data ({\tt
sp}) and time series data ({\tt zoo} and {\tt xts}).  Coercion to the
appropriate reduced spatial and temporal classes is provided, as well
as to \code{data.frame} objects in the obvious long or wide format.
...
for aggregation over time and a very flexible syntax to
...
time can be provided in two so-called wide forms. An example where
a single column refers to a single moment in time is found in the
SIDS data set:
<<>>=
library(foreign)
read.dbf(system.file("shapes/sids.dbf", package="maptools"))[1:5,c(5,9:14)]
@
where e.g. the column \code{SID74} refers to the number of sudden
infant death syndrome cases per county in 1974.

In the Irish wind data, for which the first six records contain
<<>>=
data(wind, package = "gstat")
wind[1:6,]
@
where each column refers to a wind measurement location, and the
rows reflect a single moment; wind is reported as daily average
wind speed in knots (1 knot = 0.5418 m/s).

Finally, panel data are shown in long form, where the full spatio-temporal
information is held in a single column. In the \code{Produc} data set 
(Baltagi, 2001), a panel of 48 observations from 1970 to 1986, 
the first five records are
<<>>=
data("Produc", package = "plm")
Produc[1:5,]
@
where the first two columns denote space and time, and
e.g. \code{pcap} reflects private capital stock.

None of these examples documents has referenced spatial or temporal
information: it is from the data not clear whether \code{1970}
refers to a year, or \code{ALABAMA} to a state, and where this is.
Section \ref{sec:cases} demonstrates for each of these three cases
how the data can be converted into classes with referenced space
...
Space-time sparse \code{data.frame}s (STSDF, figure \ref{fig:stsdf})
are those where time and space points of measured values can have
arbitrary organization: for each measured value the spatial location
and time point is stored. This is equivalent to a partial grid
where the index for observation $k$ is $[k,k]$, and hence can be
dropped. For these objects, $n=m$ equals the number of records.
Locations and time points need not be unique, but will be replicated
...
\subsection{Coercion between \code{ST} classes}
...
over the polygon. For gridded data, it is ambivalent whether the value
...
@
\end{center}
\caption{North Carolina sudden infant death syndrome data}
\label{fig:nc}
\end{figure}
...
@
The plot itself is left out for reasons of disk space. Time and
...
is passed on to {\tt STSDF}, and this will cause the geometries
