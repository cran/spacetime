...
spatial data taken from package \pkg{sp}, and all temporal classes
supported by package \pkg{xts}. The goal is to cover a number of
useful representations for spatio-temporal sensor data, and results
...
time-wide and space-wide formats.  We discuss when it is appropriate
to store temporal data as just a start time or as a time interval.
This document is the main reference for the \proglang{R} package
\code{spacetime}, and is available (in updated form) as a vignette
in this package. }
...
(\code{STT}), plotted against a common time axis}
\label{fig:st}
\end{figure}
...
have many missing values, or if a limited set of spatial features
...
spatial features varies (locations of crimes registered per year,
...
points are grouped, and form a trajectory. The grouping may be simple
(e.g. the trajectories of two persons on a single day), nested (for
several objects, a set of trajectories representing different trips)
or complex (e.g. with objects that split, merge, or disappear).
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=.6\columnwidth]{cls}
\end{center}
\caption{classes for spatio-temporal data in package \pkg{spacetime}. Arrows
denote inheritance, lower side of boxes list slot name and type }
\label{cls}
\end{figure}
% is.regular. or not.
...
\subsection{Methods}
The main methods for spatio-temporal data implemented in packages \pkg{spacetime}
are listed in table \ref{methods}.
\begin{table}
\begin{center}
...
\code{[[}, \code{\$}, \code{\$<-} & Selects or replaces data values \\
\code{[}, \code{[[}, \code{\$} & Select spatial and/or temporal sections \\
\code{as} & coerces to other spatio-temporal objects, \code{xts}, \code{Spatial},
\code{matrix}, or \code{data.frame} \\
\code{stplot} & creates spatio-temporal plots, see section \ref{plots}  \\
\code{over} & overlay: retrieves index or data values of one object at the locations/times
of another \\
\code{aggregate} & aggregates data values over particular spatial, temporal,
...
Aggregation of data values to a more coarse spatial or temporal form
...
\code{ST}. Grouping can be done based on spatial, temporal,
or spatio-temporal predicates. This effectively provides an
spatio-temporal equivalent to what is known in geographic information
...
or \code{data.frame}.  For a \code{data.frame}, a construct like
...
@
would select the \code{PM10} measurements for the years 2008-9,
lying in \code{Germany}, when \code{Germany} is a \code{Spatial}
object (polygons, or a grid) that defines Germany.
...
colors may denote different stations (\code{mode="ts"})
\end{itemize}
...
Hovm\"{o}ller plots \citep{hov} are an example of these for full space-time lattices,
...
define and compute with time intervals (e.g., \cite{allen}). It does
not provide structures to attach these intervals to time series data,
or else to use them in \code{xts} objects.
...
cases are accomodated for in package \pkg{spacetime}. By default,
objects deriving from \code{STF} or \code{STS} are considered
to reflect interval time, and objects deriving from \code{STI}
or \code{STT} instance time. As only start times of time intervals
are given, the package makes the assumption that the last interval
(for which no end time instance is present) has the same length as
the one-but-last interval.
...
time intervals, and \code{y} reflects time instances.  Note the
interval extended beyond the last point of \code{x}.  }
\label{fig:ti}
\end{figure}
...
@
the plot of which is shown in Figure \ref{fig:produc}.
...
@
And now we can construct the spatio-temporal object from the from space-wide table
with velocities:
<<>>=
...
separable exponential covariance model, with ranges 750 km and 1.5 day:
<<>>=
...
plot(x, add=TRUE, col = "red")
@
\caption{Single trajectory of a seal}
\label{fig:trip}
\end{figure}
