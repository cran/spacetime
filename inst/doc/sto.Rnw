...
\end{description}
All these conditions fail for intervals having zero width, i.e. the
case where $T$ is interval and for some $i$, $t_{i+1}-t_i=0$ or the
case where $U$ is interval and for some $j$, $u_{j+1}-u_j=0$.
For that reason, specifying objects with 
<<eval=FALSE>>=
